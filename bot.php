...
if (preg_match('/^\/([Ss]tart)/', $text) or $text == '⤵️ برگرد به منوی اصلی ' or $text == '🔙بازگشت به منوی اصلی' or $data == 'mainMenu') {
    ...
    if(isset($data) and $data == "mainMenu"){
        $res = editText($message_id, 'سلاااااام به ربات ما خیلی خوش اومدی 🫡🌸

ما اینجاییم تا شما را بدون هیچ محدویتی به شبکه جهانی متصل کنیم ❤️

✅ بهترین عملکرد در ارائه سرویس
...
', $mainKeys);
        if(!$res->ok){
            sendMessage('سلاااااام به ربات ما خیلی خوش اومدی 🫡🌸
...
به نظرم یه پیام براش بفرست مثلا ( تبلیغی یا خوش آمد گویی ) 😍
        ...
        sendMessage('سلااام به ربات ما خیلی خوش اومدی 🫡🌸
...
if($data=="increaseMyWallet"){
    delMessage();
    sendMessage("🙂 عزیزم مقدار شارژ مورد نظر خود را به تومان وارد کن (بیشتر از 60000 تومان)",$cancelKey);
    setUser($data);
}
if($userInfo['step'] == "increaseMyWallet" && $text != $cancelText){
    ...
    elseif($text < 60000){
        sendMessage("لطفا مقداری بیشتر از 60000 وارد کن");
        exit();
    }
